...
class Panchanga(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.computeBtn = wx.Button(self, wx.ID_ANY, u"Show Pa\xf1c\u0101\u1e45ga")
        self.prevBtn = wx.Button(self, wx.ID_ANY, u"\u25c0 Prev Day")
        self.nextBtn = wx.Button(self, wx.ID_ANY, u"Next Day \u25b6")
        self.infoTxt = wx.StaticText(self, wx.ID_ANY, "")
        ...
        self.shraddhaTithiTxt = wx.StaticText(self, wx.ID_ANY, u"dv\u0101da\u1e63\u012b")
        ...
        self.Bind(wx.EVT_TEXT_ENTER, self.calculate_panchanga, self.placeTxt)
        ...
        self.Bind(wx.EVT_BUTTON, self.on_prev_day, self.prevBtn)
        self.Bind(wx.EVT_BUTTON, self.on_next_day, self.nextBtn)
    ...
    def __set_properties(self):
        ...
        self.SetSize((625, 640))
        self.SetToolTip("Can also be entered as: 77d 35' 37\"")
        self.placeTxt.SetToolTip("Type city name. If the city is not found, enter Latitude, Longitude and Time zone directly below.")
        self.placeTxt.SetFocus()
        self.dateTxt.SetToolTip("Enter date. Negative years are treated as per proleptic Gregorian calendar.")
        self.latTxt.SetToolTip("Can also be entered as: 12d 58' 19\"")
        self.tzTxt.SetToolTip("In hours. Positive values are east of UTC and negative, west of UTC.")
        # end wxGlade
    ...
    def __do_layout(self):
        ...
        label_2 = wx.StaticText(self, wx.ID_ANY, u"D\u1e5bg-ga\u1e47ita Pa\xf1c\u0101\u1e45ga", style=wx.ALIGN_CENTRE)
        ...
        label_6 = wx.StaticText(self, wx.ID_ANY, "City")
        ...
        nav_sizer.Add(self.prevBtn, 0, wx.RIGHT, 4)
        nav_sizer.Add(self.nextBtn, 0, 0, 0)
        grid_sizer_1.Add(nav_sizer, 0, wx.ALIGN_CENTER_VERTICAL, 0)
        ...
        grid_sizer_1.Add(self.lonTxt, 0, wx.ALIGN_CENTER_HORIZONTAL | wx.ALIGN_CENTER_VERTICAL, 0)
        ...
        grid_sizer_1.Add(wx.StaticText(self, wx.ID_ANY, ""), 0, 0, 0)
        self.infoTxt.SetFont(wx.Font(10, wx.DEFAULT, wx.ITALIC, wx.NORMAL, 0, ""))
        self.infoTxt.SetForegroundColour(wx.Colour(0x33, 0x66, 0x99))
        grid_sizer_1.Add(self.infoTxt, 0, wx.ALIGN_CENTER_HORIZONTAL | wx.ALIGN_CENTER_VERTICAL, 0)
        grid_sizer_1.Add(wx.StaticText(self, wx.ID_ANY, ""), 0, 0, 0)
        ...
        label_st = wx.StaticText(self, wx.ID_ANY, u"\u015ar\u0101ddha Tithi")
        grid_sizer_1.Add(label_st, 0, wx.ALIGN_CENTER_HORIZONTAL | wx.ALIGN_CENTER_VERTICAL, 0)
        grid_sizer_1.Add(self.shraddhaTithiTxt, 0, wx.ALIGN_CENTER_VERTICAL, 0)
        grid_sizer_1.Add(wx.StaticText(self, wx.ID_ANY, ""), 0, 0, 0)
    ...
    def calculate_panchanga(self, event):  # wxGlade: Panchanga.<event_handler>
        city_name = self.placeTxt.Value.strip()
        if city_name:
            self.resolve_city(city_name)
        ...
        friendly_date = dt.strftime("%A, %d %B %Y")
        display_city = city_name.title() if city_name else "custom location"
        self.infoTxt.SetLabel(u"Pa\xf1c\u0101\u1e45ga for %s \u2014 %s" % (friendly_date, display_city))
        ...
        self.shraddhaTithiTxt.SetLabel(self.tithis[str(st)])
    ...
    def _shift_date(self, days):
        date = self.parse_date()
        dt = datetime(date.year, date.month, date.day) + timedelta(days=days)
        self.dateTxt.SetValue("%d/%d/%d" % (dt.day, dt.month, dt.year))

    def on_prev_day(self, event):
        self._shift_date(-1)
        self.calculate_panchanga(event)
    ...
    def resolve_city(self, city_name):
        ...
        if city in self.cities:
            ...
            self.tzone = timezone(info['timezone'])
            tz_offset = self.compute_timezone_offset()
            self.place = Place(info['latitude'], info['longitude'], tz_offset)
            self.latTxt.SetValue("%.5f" % info['latitude'])
            self.lonTxt.SetValue("%.5f" % info['longitude'])
