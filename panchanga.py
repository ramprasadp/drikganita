...
def sunrise(jd, place):
  ...
  result = swe.rise_trans(jd - tz/24, swe.SUN, rsmi, (lon, lat, 0.0))
...
def moonrise(jd, place):
  ...
  lat, lon, tz = place
  rsmi = swe.BIT_DISC_CENTER + swe.CALC_RISE
  result = swe.rise_trans(jd - tz/24, swe.MOON, rsmi, (lon, lat, 0.0))
  rise = result[1][0]  # julian-day number
  # Convert to local time
...
def tithi_at_time(jd_local, place):
    """
    Return tithi number at a given LOCAL julian time for a place.
    """
    jd_ut = jd_local - place.timezone / 24.0

    phase = lunar_phase(jd_ut)
    t = int(floor(phase / 12)) + 1
    if t > 30:
        t = 30

    return t

def shraddha_tithi(jd, place):

    sr = sunrise(jd, place)[0]
    ss = sunset(jd, place)[0]

    day_len = ss - sr
    part = day_len / 5

    # Aparahna midpoint
    aparahna = sr + 3.5 * part

    return tithi_at_time(aparahna, place)
